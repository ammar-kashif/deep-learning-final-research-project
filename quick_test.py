...
import config
config.NUM_EPOCHS = 1  # Quick test: 1 epoch for fast validation (use main.py for full 50 epochs)
config.CROSS_DATASET_SAMPLE_SIZE = 10000  # Sample size for cross-dataset validation
...
def main():
    """Quick test of the complete pipeline with full dataset (1 epoch for fast validation)."""
    logger.info("=" * 80)
    logger.info(f"QUICK TEST MODE - Running pipeline with full dataset, {NUM_EPOCHS} epoch(s)")
    logger.info("Note: Using 1 epoch for fast validation. For full training (50 epochs), run: python3 main.py")
    logger.info("=" * 80)
    ...
    try:
        ...
        # Use full dataset for quick test (1 epoch for fast validation)
        logger.info(f"Using full dataset for quick test (1 epoch for fast validation)")
        ...
        # Use configured sample size for cross-dataset validation (for computational efficiency)
        cross_sample_size = min(config.CROSS_DATASET_SAMPLE_SIZE, len(df_b_translated))
        ...
        logger.info(f"Computing ELECTRA-BERT agreement on sample of {cross_sample_size} (from {len(df_b_translated)} total)")
        # Extract sentiment features for Dataset B if needed
        X_test_b_sentiment = None
        if USE_SENTIMENT_FEATURES:
            logger.info("Extracting sentiment features for Dataset B")
            sentiment_extractor = sent_features.SentimentFeatureExtractor()
            X_test_b_sentiment = sentiment_extractor.extract_features(X_test_b)
        
        predictions_electra_b = electra_trainer.predict(X_test_b, X_test_sentiment=X_test_b_sentiment)
        predictions_bert_b = bert_trainer.predict(X_test_b, X_test_sentiment=X_test_b_sentiment)
